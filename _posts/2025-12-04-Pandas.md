...
Pandas use the `loc` attribute to return one or more specified row(s) & column(s)
...
# Return row 0 and 1 and column "duration"
print(df.loc[[0, 1], ["duration"]])
#    duration
# 0        50
# 1        40

# only select columns, : means select all rows
...
### Filter DataFrame Using Boolean Columns

when passing a boolean column to a DataFrame, it will filter for rows where value is `True`
...
Use `iterrows()` method to iterate over a Pandas DataFrame
